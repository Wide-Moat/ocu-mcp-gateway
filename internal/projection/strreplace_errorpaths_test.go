...
func runStrReplace(t *testing.T, path, oldStr, newStr string) (string, bool) {
	...
	failed := false
	if runErr != nil {
		// errors.As, not a type assertion: a wrapped ExitError would otherwise be
		// misread as "the interpreter failed to run", turning a correct refusal
		// into a t.Fatalf and hiding the very semantics this test pins.
